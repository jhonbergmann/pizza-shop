...
export const updateMenu = new Elysia().use(authentication).put(
  ...
  async ({getManagedRestaurantId, set, body}) => {
    ...
    if (deletedProductIds.length > 0) {
      await db
        .delete(products)
        .where(and(inArray(products.id, deletedProductIds), eq(products.restaurantId, restaurantId)))
    }
    ...
    const updatedProducts = newOrUpdatedProducts.filter((product): product is ProductWithId => {
      return !!product.id
    })
    ...
    const newProducts = newOrUpdatedProducts.filter((product): product is ProductWithoutId => {
      return !product.id
    })
