...
export const getMonthReceipt = new Elysia()
  ...
  .get('/metrics/month-receipt', async ({getManagedRestaurantId}) => {
    ...
    const monthsReceipts = await db
      ...
      .from(orders)
      .where(and(eq(orders.restaurantId, restaurantId), gte(orders.createdAt, startOfLastMonth.toDate())))
    ...
    const diffFromLastMonth =
      lastMonthReceipt && currentMonthReceipt ? (currentMonthReceipt.receipt * 100) / lastMonthReceipt.receipt : null
