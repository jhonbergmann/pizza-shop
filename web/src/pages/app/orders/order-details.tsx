    ...
    <DialogContent className="sm:max-w-[520px]">
      <DialogHeader>
        <DialogTitle className="flex items-center gap-2">
          Pedido: {orderId}
          {isFetchingOrder && <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />}
        </DialogTitle>
            ...
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell className="text-right">
                  {order.customer.phone ?? <span className="italic text-muted-foreground">Não informado</span>}
                </TableCell>
              </TableRow>
              <TableRow>
              ...
              {order.orderItems.map((orderItem) => {
                return (
                  <TableRow key={orderItem.id}>
                    ...
                    <TableCell className="text-right">
                      {((orderItem.priceInCents * orderItem.quantity) / 100).toLocaleString('pt-BR', {
                        style: 'currency',
                        currency: 'BRL',
                      })}
                    </TableCell>
